...
inline std::vector<osuCrypto::block> rpir_batched_receiver_ngc(std::vector<std::vector<Channel>> chls, std::vector<std::vector<osuCrypto::block>> inputSet)
{	
	...
	// std::cout<<"nums to compare: "<<numbers_to_compare.size()<<std::endl;
...
inline std::vector<osuCrypto::block> rpir_batched_receiver_ngc_multiThreads(std::vector<std::vector<Channel>> chls, std::vector<std::vector<osuCrypto::block>> inputSet,int numThreads)
{	
	AES hashOKVS(toBlock(12138));
	PRNG secret_value_generater(_mm_set_epi32(4253465, 12365, 234435, 23987054));
	BitVector choices(inputSet.size());
...
inline std::vector<std::array<osuCrypto::block, 2>> rpir_batched_sender_ngc_multiThreads(std::vector<std::vector<Channel>> chls, std::vector<osuCrypto::block> inputSet, u64 maxBinSize,int numThreads)
{	
	// sender
	AES hashOKVS(toBlock(12138));
...
inline std::vector<std::array<osuCrypto::block, 2>> rpir_batched_sender_ngc(std::vector<std::vector<Channel>> chls, std::vector<osuCrypto::block> inputSet, u64 maxBinSize)
{	
	// sender
	AES hashOKVS(toBlock(12138));
	// BitVector b_s_vec(inputSet.size());
